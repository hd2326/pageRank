...
bubble_plot <- function(s_mat, c_mat, n_mat, col=colorRampPalette(c("Blue", "Grey", "Red"))(100),
  ...
  for (i in seq_len(ncol(s_mat))) points(rep(i, nrow(s_mat)), seq_len(nrow(s_mat)), cex = s_mat[, i], col = c_mat[, i], pch = 16)
  for (i in seq_len(ncol(n_mat))) text(rep(i+0.5, nrow(n_mat)), seq_len(nrow(n_mat)), labels = n_mat[, i])
  axis(side = 1, at = seq_len(length(col_names)), labels = col_names, tick = FALSE, las = 2)
  axis(side = 2, at = seq_len(length(row_names)), labels = row_names, tick = FALSE, las = 2)}
